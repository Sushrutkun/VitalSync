 ...
 *
 * One sync request produces exactly one Kafka message:
 *   - key: userId  (guarantees per-user ordering on a single partition)
 *   - value: the {@link HealthSyncRequest} payload
 *   - header "idempotency-key": carried through so consumers can dedupe retries
 */
@Service
@Slf4j
...
public class HealthSnapshotPublisher {
    ...
    public void publish(HealthSyncRequest request) {
        ProducerRecord<String, Object> record = new ProducerRecord<>(
                topicName, null, request.userId(), request);
        ...
        try {
            ...
            log.info("Published snapshot user=[{}] idempotencyKey=[{}] partition=[{}] offset=[{}]",
                    request.userId(),
            ...
            throw new KafkaPublishException(
                    "Failed to publish snapshot for user " + request.userId(), e);
        }
    }
