...
public class AuthService {
    ...
    public AuthResponse login(LoginRequest req) {
        User u = users.findByEmail(req.email())
                .orElseThrow(() -> new AuthException(AuthErrorCode.INVALID_CREDENTIALS, "Email or password is incorrect"));
    ...
    /**
     * §7.2 rotation: lookup by token id, verify secret, delete row, issue new pair.
     * Token format on the wire: "{id}.{secret_hex}". Only secret is bcrypt-hashed in DB.
     */
    @Transactional
    public AuthResponse refresh(String rawToken) {
        ParsedToken p = parse(rawToken);
        RefreshToken match = refreshTokens.findById(p.id)
                .orElseThrow(() -> new AuthException(AuthErrorCode.REFRESH_TOKEN_INVALID, "Refresh token not found"));
        ...
        User u = users.findById(match.getUserId())
                .orElseThrow(() -> new AuthException(AuthErrorCode.REFRESH_TOKEN_INVALID, "User no longer exists"));
        return issueTokens(u);
    }
    ...
    public void logout(String rawToken, String authUserId) {
        ...
        refreshTokens.findById(p.id).ifPresent(rt -> {
